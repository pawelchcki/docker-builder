...
def test_basic_file_building():
    ...
    assert (
        "66ba00ad3de8677a3fa4bc4ea0fc46ebca0f14db46ca365e7f60833068dd0148" == d.build()
    )


def test_local_file_inclusion_and_isolation():
    isolated_implicit = dockerfile(
        "./local_implicit_dependencies.Dockerfile"
    ).isolated_paths("./local_dependencies/A")
...
def test_extract_file_from_image():
    ...
    image_built_from_alternative_root = dockerfile(
        "./local_explicit_dependencies.Dockerfile",
        root_dir=tests_root / "local_dependencies/alternative_root",
    ).image()
...
def test_create_artifact_only_image():
    base = Image("scratch")
    result = base.modify_image(
        append_paths_mapped={"/A": "local_dependencies/A", "/AA": VirtualFile("AA")}
    )
